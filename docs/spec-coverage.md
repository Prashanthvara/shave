...
**Reviewed 2026-09-15** against the design doc produced by `/office-hours`:
...
|---|---|
| Commits | 77 |
| Source | 5,306 lines across 20 modules, plus `app.js` / `app.css` |
| Tests | **1067 Python** (4 network-marked, deselected) + **65 render** |
| Towns | **3**: Worcester, Fall River, Lowell. 4,179 parcels screened, 1,450 in band, 281 in the sweet spot |
| Live response | index + a town's `ranked.json` in **0.38 s** |
| Coverage of spec stages | **10 of 12 numbered steps complete**, 1 partial |
| Success criteria | **7 of 7** |

> The Python suite scores each town once per session (`tests/conftest.py`). It used to re-run the Worcester pipeline in seven separate tests.

> Every figure here was re-measured on 2026-09-15. The previous review undercounted commits, crosswalk rows and tests, and still said one town.
...
| Step | State | Evidence |
|---|---|---|
| 0. Pull real `USE_CODE` counts; build `crosswalk.csv` (~40 rows); 900-series as its own branch | ✅ | **178 rows** in `src/shave/crosswalk.csv`, scoped per town, `source` visible per row (81 comstock / 26 modeled / 71 excluded), 97 of them 900-series, 2 collapse points flagged |
...
|---|---|---|---|
| 1 | MassGIS L3, `TaxPar`+`Assess` joined on `LOC_ID` one-to-many, collapse rule | ✅ | **4,179 scoreable parcels across three towns**: Worcester 2,099, Fall River 1,130, Lowell 950. Each town carries its own assessor extract and its own ComStock county, and the page loads one town at a time. |
| 2 | `BLDG_AREA` null/zero handling; fall back to roofprint × stories; drop to LOW | ✅ | Missing floor area falls back to roofprint area × recorded stories (one when none is recorded) and drops to LOW with reason floor_area_from_roofprint. Exercised in practice: 8 exported rows carry `sqft_source=roofprint` (7 Fall River, 1 Lowell). A further 101 parcels have neither a recorded area nor a usable roofprint and are reported unscored rather than guessed at. No Worcester parcel needs the fallback, and the test pins that at zero. |
...
| 5 | Scoring: monthly loop, root-find, band filter, confidence tiers | ✅ | 1,450 kept and 281 in the sweet spot across three towns (Worcester 737/172, Fall River 377/54, Lowell 336/55). Seven confidence predicates. |
...
|---|---|---|---|
| 1 | Public URL, ranked list under 3 s, no signup | ✅ | **0.38 s** measured (index page 0.14 + the default town's `ranked.json` 0.24). No credential on any route. |
...
| 3 | Both R² figures, published with the explanation, 0.90 threshold declared in advance | ✅ | Across all three towns: comstock **0.463 → 0.723** (n=987), modeled **0.351 → 0.640** (n=463). Both under the ceiling, so the archetype layer earns its place. Adding two towns lowered both figures, which is the honest direction: a wider spread of building stock is harder for size and rate alone to explain. |
...
| 7 | Stage 2: paste an address in a covered town, get a dossier | ✅ | All three towns, from one index. Exact, approximate and uncovered-town answers; a hit in another covered town switches the page to that town rather than failing silently; every exported row is found by its own address (`test_addresses.py`). |

### A note on criterion 3

The spec predicted the second R² would be **≈1.000 by construction**. It is **0.723**. The rendered
...
| 2 | **A fourth town** | 2.5h | Each needs an L3 download, a crosswalk pass, its own `county_gisjoin`, and `scripts/fetch_structures.py --town-id`. The town registry and the per-town build are built for it. Not New Bedford or Chicopee: MassGIS's electricity-provider layer lists New Bedford as Eversource and Chicopee as a municipal light plant, so National Grid's G-2/G-3 tariff never applies there. Fall River and Lowell replaced them, and `data/town_utilities.csv` is the checkable record. |
...
for machine shops (06:30 shift start) and 1.22× for cold storage (06:00), covering 227 parcels, the core of
the published ICP.
...
boolean. The status field has four values, `clear`, `screened_out`, `no_roofprint` and
